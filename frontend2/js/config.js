    ...
    reglas.forEach((r, i) => {
        ...
        html += `<div class="regla-item">
            ...
            <div class="regla-actions">
                ...
                ${reglas.length > 1 ? `<button class="btn btn-danger btn-sm" onclick="eliminarRegla(${i})">X</button>` : ''}
            </div>
        </div>`;
    });
...
function eliminarRegla(idx) {
    if (config.reglas_dependencia.length <= 1) {
        return alert("No se puede eliminar. Minimo 1 regla requerida.");
    }

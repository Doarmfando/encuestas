...
READY_SELECTORS = {
    ...
    "unsupported": ['form', 'input, textarea, select, [role="radio"], [role="checkbox"], [role="listbox"]'],
}
...
def wait_for_form_ready(page, url: str = "", runtime_config: dict | None = None) -> bool:
    ...
    try:
        page.wait_for_load_state("domcontentloaded", timeout=timeout_ms or 1000)
    except Exception:
        pass

    if speed_profile == "turbo_plus":
        # Modo ultra rapido: un solo selector con timeout corto.
        try:
            page.wait_for_selector(selectors[0], timeout=min(per_selector_timeout, 250))
        except Exception:
            pass
        return True
...
def _safe_page_content(page) -> str:
    # Usar innerText (texto visible) en vez de page.content() (HTML crudo).
    # page.content() devuelve los bundles de JS de Google Forms que contienen
    # strings como "enviar otra respuesta" aunque el form no haya sido enviado,
    # lo que genera falsos positivos en la detección de envío exitoso.
    try:

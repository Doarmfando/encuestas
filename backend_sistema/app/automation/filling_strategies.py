...
class FillingStrategies:
    ...
    def click_option_by_text(container, page, valor: str, role: str = "radio",
        ...
        # Estrategia 0: match por atributo value del input (MS Forms lo usa en checkboxes)
        for idx in range(count):
            try:
                val_attr = inputs.nth(idx).get_attribute("value") or ""
                if val_attr and _norm_text(val_attr) == valor_norm:
                    return FillingStrategies._click_at_index_fallback(inputs, page, idx, use_js_click, runtime_config=runtime_config)
            except Exception:
                continue

        # Estrategia 0b: resolver aria-labelledby -> leer texto del elemento referenciado
        for idx in range(count):
            try:
                labelled_by = inputs.nth(idx).get_attribute("aria-labelledby") or ""
                if not labelled_by:
                    continue
                # aria-labelledby puede tener múltiples ids separados por espacio
                for ref_id in labelled_by.split():
                    try:
                        ref_el = page.locator(f'#{ref_id}').first
                        txt = ref_el.inner_text(timeout=300).strip()
        ...
        # Estrategia 1: match exacto (normalizado sin tildes)
        for idx, text in enumerate(option_texts):
            if _norm_text(text) == valor_norm:
                return FillingStrategies._click_at_index_fallback(inputs, page, idx, use_js_click, runtime_config=runtime_config)

        # Estrategia 2: match parcial (normalizado sin tildes)
        for idx, text in enumerate(option_texts):
            text_norm = _norm_text(text)
            if valor_norm and text_norm and (valor_norm in text_norm or text_norm in valor_norm):
                return FillingStrategies._click_at_index_fallback(inputs, page, idx, use_js_click, runtime_config=runtime_config)

        # Estrategia 3: match por aria-label del propio input/role
        for idx in range(count):
            try:
                aria = inputs.nth(idx).get_attribute("aria-label") or ""
                aria_norm = _norm_text(aria)
                if aria_norm and (aria_norm == valor_norm or valor_norm in aria_norm or aria_norm in valor_norm):
                    return FillingStrategies._click_at_index_fallback(inputs, page, idx, use_js_click, runtime_config=runtime_config)
            except Exception:
                continue

        # Estrategia 4: click directo sobre un label/span ancestro que contenga el texto exacto
        try:
            scan = container.locator('label, span, div[role="checkbox"], div[role="radio"]')
            scan_count = scan.count()
            for i in range(min(scan_count, 150)):
                try:
                    el = scan.nth(i)
        ...
        # Estrategia 5: valor numérico como índice 1-based
        ...
        # Diagnóstico: imprimir textos detectados para debuggear
        try:
    ...
    @staticmethod
    def _click_at_index_fallback(inputs, page, idx: int, use_js_click: bool = False, runtime_config: dict | None = None) -> bool:
        """Click en un control usando el input y sus wrappers visibles como fallback."""
        try:
            el = inputs.nth(idx)
            targets = [
                el,
            ...
            for target in targets:
                if not FillingStrategies._click_locator(target, page, use_js_click, runtime_config=runtime_config):
                    continue
                if FillingStrategies._requires_selected_validation(el):
                    if FillingStrategies._is_control_selected(el):
                        return True
                    continue
                return True
    ...
    def _click_locator(target, page, use_js_click: bool = False, runtime_config: dict | None = None) -> bool:
        """Ejecuta click sobre un locator con fallback JS/Playwright."""
        actions = []
        if use_js_click:
            actions.append(
    ...
    def _click_at_index(inputs, page, idx: int, use_js_click: bool = False, runtime_config: dict | None = None) -> bool:
        ...
        return FillingStrategies._click_at_index_fallback(
            inputs, page, idx, use_js_click, runtime_config=runtime_config
        )

...
class ThreadLocalStdout:
    ...
    def write(self, text):
        capture = getattr(_thread_local, "log_capture", None)
        if capture:
            capture.write(text)
        else:
...
class ExecutionService:
    ...
    # Instalar el stdout thread-safe una sola vez
    ...
    def execute(
        ...
        with app.app_context():
            # Registrar el LogCapture en este hilo
            _thread_local.log_capture = log_capture
            try:

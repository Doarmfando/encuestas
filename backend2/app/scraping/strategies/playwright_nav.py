...
class PlaywrightNavStrategy:
    ...
    def extract(self, page, url: str) -> dict:
        ...
        while num_pagina <= max_paginas:
            try:
                ...
                for item in items_check:
                    try:
                        ...
                        pass
                hash_pagina = hash(tuple(sorted(textos_items)))
            except Exception:
            ...
            for item in items:
                ...
                if pregunta:
                    texto_key = pregunta["texto"].strip().lower()[:40]
                    if texto_key in preguntas_vistas and pregunta["tipo"] != "informativo":
                        continue
                    preguntas_vistas.add(texto_key)
                    pagina_data["preguntas"].append(pregunta)
                    resultado["total_preguntas"] += 1

            if not pagina_data["preguntas"]:
                botones = detectar_botones(page, url)
            ...
            pagina_data["botones"] = detectar_botones(page, url)
    ...
    def _extraer_pregunta(self, item) -> dict | None:
        ...
        try:
            ...
            try:
                file_inputs = item.locator(
                    'input[type="file"], [aria-label*="Subir"], [aria-label*="Upload"], '
                    'button:has-text("Agregar archivo"), button:has-text("Add file")'
                )
